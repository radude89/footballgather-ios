...
class GatherViewController: UIViewController {
    ...
    enum GatherCountDownTimerComponent: Int, CaseIterable {
    ...
    private func updateGather(_ gather: GatherCreateModel, completion: @escaping (Bool) -> Void) {
        ...
        updateGatherService.update(gather, resourceID: ResourceID.uuid(gatherModel.gatherUUID)) { result in
        ...
        updateGather(gather) { [weak self] gatherWasUpdated in
            ...
            DispatchQueue.main.async {
                ...
                if !gatherWasUpdated {
                    ...
                    guard let playerListTogglable = self.navigationController?.viewControllers.first(where: { $0 is PlayerListTogglable }) as? PlayerListTogglable else {
                        return
                    }
                    ...
                    if let playerListViewController = playerListTogglable as? UIViewController {
                        self.navigationController?.popToViewController(playerListViewController, animated: true)
                    }
                }
            }
        }
