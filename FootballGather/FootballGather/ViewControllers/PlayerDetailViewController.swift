...
final class PlayerDetailViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        guard segue.identifier == SegueIdentifier.editPlayer.rawValue,
            ...
            let selectedPlayerRow = viewModel.selectedPlayerRow else {
                return
        }
        
        destinationViewController.viewModel = makeEditViewModel(from: selectedPlayerRow)
        destinationViewController.delegate = self
    }
    
    private func makeEditViewModel(from selectedPlayerRow: PlayerRow) -> PlayerEditViewModel {
        let editViewModel = PlayerEditViewModel(viewType: viewModel.destinationViewType,
                                                   playerEditModel: PlayerEditModel(player: viewModel.player, playerRow: selectedPlayerRow))
        
        if viewModel.shouldEditSelectionTypeField {
            let itemsEditModel = PlayerItemsEditModel(items: viewModel.editableItems, selectedItemIndex: viewModel.indexOfSelectedItem ?? -1)
