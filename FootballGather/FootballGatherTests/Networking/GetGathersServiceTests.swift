...
final class GetGathersServiceTests: XCTestCase {
    ...
    func test_request_completesSuccessfully() {
        ...
        service.get { (result: Result<[GatherResponseModel], Error>) in
            switch result {
            case .success(let gathers):
                XCTAssertGreaterThan(gathers.count, 1)
                exp.fulfill()
            case .failure(_):
